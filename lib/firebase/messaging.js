...
import { getStoredPushToken, setStoredPushToken, arePushAlertsDisabled } from '../push/tokenStore';
...
function attachForegroundNotificationHandler(messaging) {
  ...
  onMessage(messaging, async (payload) => {
    ...
    // Do not display notification if disabled by user preference
    if (localStorage.getItem('cafeqr_notifications_enabled') === 'false' || arePushAlertsDisabled()) {
      return;
    }
    ...
    if (Capacitor.isNativePlatform()) {
      ...
      return new Promise((resolve, reject) => {
        ...
        const errorListener = PushNotifications.addListener('registrationError', (error) => {
          ...
          successListener.remove();
          errorListener.remove();
          reject(new Error(error.error || 'Failed to register native push'));
        });
