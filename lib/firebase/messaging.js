...
function attachForegroundNotificationHandler(messaging) {
  ...
  onMessage(messaging, async (payload) => {
    ...
    try {
      ...
      const soundAlertEnabled = localStorage.getItem('sound_alert_enabled') !== '0';
      if (soundEnabled && soundAlertEnabled) {
        const audio = new Audio('/beep.mp3');
        audio.play().catch((err) => {
          console.warn('[push:web] Sound play blocked by browser policy:', err);
        });
      }
    } catch (err) {
      console.warn('[push:web] Sound play failed:', err);
    }
    ...
    if (Notification.permission === 'granted') {
      ...
      const displayBody = itemsSummary ? `${body}\n${itemsSummary}` : body;
      ...
      try {
        ...
        console.warn('[push:web] Failed to show native Notification, trying SW:', err);
